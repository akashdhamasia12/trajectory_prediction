...
def trajectories_plot(image, outputs, targets, j, image_agent_all, centroid_, ade, fde, history_traj, maneuver_label, maneuver_pred):
    """
    ...
    ax1.plot(outputs[:,0], outputs[:,1], color='red', marker='x', linewidth=0.1, markersize=1.4, label='Predictions')
    ...
    ax2.plot(outputs[:,0], outputs[:,1], color='red', marker='x', linewidth=0.1, markersize=1.4, label='Predictions')
    ax2.plot(history_traj[:,0], history_traj[:,1], color='cyan', marker='+', linewidth=0.1, markersize=1, label='history')

    ax3.plot(targets[:,0], targets[:,1], color='green', marker='o', label='Ground-Truth')
    ax3.plot(outputs[:,0], outputs[:,1], color='red', marker='x', label='Predictions')
    ax3.plot(history_traj[:,0], history_traj[:,1], color='cyan', marker='+', label='history')
...
if config.MULTIPLE_GPU:
    model = torch.nn.DataParallel(model, device_ids=[1,0]).cuda()
else:
...
with torch.no_grad():
    for i, data in enumerate(test_loader):
        ...
        ade = ade * config.IMAGE_FACTOR
        fde = fde * config.IMAGE_FACTOR
        ...
        trajectories_plot(image, outputs, keypoints, seq_id.item(), image_agent, centroid_current, ade, fde, history_traj, maneuver_label, maneuver_pred)
