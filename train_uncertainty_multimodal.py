...
prior_mu = 0.0
prior_sigma = 0.1 #should increase it. 0.1 to 0.5
posterior_mu_init = 0.0
posterior_rho_init = -9.0 #-1000.0 #-9.0 #-6.0
...
det_checkpoint = torch.load("/home/neslihan/akash/datasets/lyft_10_30_9000_balanced/outputs_multimodal/best_epoch_MSE_.pth")
...
def fit(model, dataloader, data, epoch):
    ...
    return train_loss, train_loss_mse, train_loss_kl
...
def validate(model, dataloader, data, epoch):
    ...
    return valid_loss, val_loss_mse, val_loss_kl
...
for epoch in range(begin_epoch, config.EPOCHS):
    ...
    train_epoch_loss, train_mse_loss, train_kl_loss = fit(model, train_loader, train_data, epoch)
    val_epoch_loss, val_mse_loss, val_kl_loss = validate(model, valid_loader, valid_data, epoch)
    ...
    f.write(str(epoch)+","+str(train_epoch_loss)+","+str(val_epoch_loss)+","+str(config.LR)+","+str(best_epoch_no)+","+str(best_val_loss)+","+str(train_mse_loss)+","+str(train_kl_loss)+","+str(val_mse_loss)+","+str(val_kl_loss)+"\n")
