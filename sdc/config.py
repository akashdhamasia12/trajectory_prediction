...
def build_parser():
    ...
    parser.add_argument(
        ...
        help='Directory where SDC data is stored. We also use this to cache '
             'torch hub models.')
    parser.add_argument(
        '--dir_metrics', type=str, default='metrics',
        help='Directory where SDC data is stored. We also use this to cache '
             'torch hub models. Default will '
             'be set to `{dir_data}/metrics`.')
    parser.add_argument( # check again!
    ...
    parser.add_argument(
        ...
        help='Use TensorBoard logging for losses and visualizations.') #Important!
    parser.add_argument(
        '--np_seed', type=int, default=20,
        help='NumPy seed (data processing, train/test splits, etc.).')
    parser.add_argument(
        '--torch_seed', type=int, default=20,
        help='Model seed.')
    ...
    ###########################################################################
    parser.add_argument(
        '--data_num_workers', type=int, default=10,
    ...
    parser.add_argument(
        ...
        default=True,
        type='bool', help='Use pre-rendered data.') #Important argument!!
    ...
    parser.add_argument(
        ...
        help=f'Specify an explicit name for the experiment - otherwise we '
             f'generate a unique wandb ID.') #Test this one!
    parser.add_argument(
    ...
    parser.add_argument(
        '--exp_batch_size', type=int, default=256) #default=256
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Loss to use for model checkpointing (i.e., checkpoint if model '
             ...
             'the in-distribution (Moscow, no precipitation) validation set.') #Check if it is still ade for stochastic case as well?
            #check this loss for full dataset!!
    ###########################################################################
    # #### Model ##############################################################
    ###########################################################################
    ...
    parser.add_argument(
        ...
        help="Optionally specify the name of the subdirectory to "
             "which we checkpoint.")
    parser.add_argument( #output of mobilenetv2
    ...
    parser.add_argument(
        ...
        help='Train BC on the ADE objective, instead of using a valid '
             'likelihood.')
    parser.add_argument( #Train BS using valid likelihood
        ...
        help='Specifies the mode used in generating trajectories from DIM/BC '
             ...
             'sampling: condition on samples.') #check autoregressive likelihood!
    parser.add_argument(
        '--dim_scale_eps', type=float, default=1e-7,
        help="Additive epsilon constant to avoid a 0 or negative scale.")
    parser.add_argument( #better understand this item!!
