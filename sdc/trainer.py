...
def count_parameters(model):
    ...
    """
    return sum(p.numel() for p in model.parameters() if p.requires_grad) #trainable parameters
...
def train(c):
    ...
    # RIP wrapper if specified (e.g., model averaging), --> what does model averaging do?
    ...
    sdc_loss = SDCLoss(full_model_name=full_model_name, c=c) #Important!!
    ...
    with tq.tqdm(range(num_epochs)) as pbar_epoch:
        for epoch in pbar_epoch:
            ...
            pbar_string = ''
            f=open("/home/neslihan/VehicleMotionPrediction/shifts/sdc/metrics/bc/train_nll_ade_fde.txt","a+")
            ...
            f=open("/home/neslihan/VehicleMotionPrediction/shifts/sdc/metrics/bc/val_nll_ade_fde.txt","a+")
            if consider_eval:
            ...
            #f.write(str(epoch_loss_dict), ",", str(steps), ",", str(epoch), "\n")
            if not eval_mode and consider_train:
                scheduler.step()
