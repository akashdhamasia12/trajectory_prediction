...
image_h_w = 300
to_resize = 224
image_factor = 224/300
print("NGSIM2LYFT")

history_old = 10 #(9 history + 1 current)
history = 11
future_pred_old = 19#19 #8 #30: creating shifted dataset
future_pred = 30#19 #8 #30: creating shifted dataset
    ...
    if len(files) == 24:
        ...
        for i in range(0, history_old):
            ...
            if im_agent is not None and im_target is not None:
                ...
                im_agent = cv2.resize(im_agent, (to_resize, to_resize))
                im_target = cv2.resize(im_target, (to_resize, to_resize))
        ...
        for i in range(0, future_pred):
            ...
            if i+1 < future_pred_old:
                if prev_frame == int(data[0][i+1]):
                    availability = 0
                else:
                    availability = 1
                y_frame =  (image_h_w/2 - data[1][i+1])*image_factor #x & y are reversed in NGSIM #i+1 because first frame is current frame
                x_frame =  (image_h_w/2 - data[2][i+1])*image_factor
