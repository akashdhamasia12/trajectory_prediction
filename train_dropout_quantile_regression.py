...
def fit(model, dataloader, data, epoch):
    ...
    for i, data in tqdm(enumerate(dataloader), total=num_batches):
        ...
        if config.num_monte_carlo_training > 1:
            ...
            total_uncertainty = torch.mean(outputs_var_mean, 1)
            total_uncertainty = torch.mean(total_uncertainty)
    ...
    return train_loss, train_ade, train_fde, train_uncertainty
...
def validate(model, dataloader, data, epoch):
    ...
    return valid_loss, val_ade, val_fde, val_uncertainty
...
for epoch in range(begin_epoch, config.EPOCHS):
    ...
    train_epoch_loss, train_ade, train_fde, train_uncertainty = fit(model, train_loader, train_data, epoch)
    val_epoch_loss, val_ade, val_fde, val_uncertainty = validate(model, valid_loader, valid_data, epoch)
    if config.DATASET == "SHIFT":
    ...
    f.write(str(epoch)+","+str(train_epoch_loss)+","+str(val_epoch_loss)+","+str(learning_rate)+","+str(best_epoch_no)+","+str(best_val_loss)+","+str(train_ade)+","+str(train_fde)+","+str(val_ade)+","+str(val_fde)+","+str(val_ade_out)+","+str(val_fde_out)+","+str(train_uncertainty)+","+str(val_uncertainty)+","+str(val_uncertainty_out)+"\n")
