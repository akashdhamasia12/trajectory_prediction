...
model_1 = build_model().to(config.DEVICE)
# print(model)

if config.MULTIPLE_GPU:
    model_1 = torch.nn.DataParallel(model_1, device_ids=[0,1]).cuda()
else:
    model_1 = torch.nn.DataParallel(model_1, device_ids=[0]).cuda()

model_1.to(config.DEVICE)

# # load the model checkpoint
print('Loading checkpoint')
checkpoint = torch.load(f"{config.model_1_path}")
model_epoch = checkpoint['epoch']
# load model weights state_dict
model_1.load_state_dict(checkpoint['model_state_dict'])#, strict=False) # Error(s) in loading state_dict for ResNet: -> added strict-False
print('Loaded checkpoint at epoch', model_epoch)

##################################################################################################

model_2 = build_model().to(config.DEVICE)
# print(model)

if config.MULTIPLE_GPU:
    model_2 = torch.nn.DataParallel(model_2, device_ids=[0,1]).cuda()
else:
    model_2 = torch.nn.DataParallel(model_2, device_ids=[0]).cuda()

model_2.to(config.DEVICE)

# # load the model checkpoint
print('Loading checkpoint')
checkpoint = torch.load(f"{config.model_2_path}")
model_epoch = checkpoint['epoch']
# load model weights state_dict
model_2.load_state_dict(checkpoint['model_state_dict'])#, strict=False) # Error(s) in loading state_dict for ResNet: -> added strict-False
print('Loaded checkpoint at epoch', model_epoch)


# ##################################################################################################
# model_3 = build_model().to(config.DEVICE)
# # print(model)

# if config.MULTIPLE_GPU:
#     model_3 = torch.nn.DataParallel(model_3, device_ids=[0,1]).cuda()
# else:
#     model_3 = torch.nn.DataParallel(model_3, device_ids=[0]).cuda()

# model_3.to(config.DEVICE)

# # # load the model checkpoint
# print('Loading checkpoint')
# checkpoint = torch.load(f"{config.model_3_path}")
# model_epoch = checkpoint['epoch']
# # load model weights state_dict
# model_3.load_state_dict(checkpoint['model_state_dict'])#, strict=False) # Error(s) in loading state_dict for ResNet: -> added strict-False
# print('Loaded checkpoint at epoch', model_epoch)
    ...
    for i, data in enumerate(test_loader):
        ...
        for model in model_list:
            ...
            if ade_value < ade_final:
                ade_final = ade_value
                ade_index = counter_model

            if fde_value < fde_final:
                fde_final = fde_value
                fde_index = counter_model
