...
complete_map = cv2.imread("complete_map_providentia.png")
dim = (224*3, 224*3)
...
get_map1 = np.zeros((224,224,3), np.uint8)

color = [(0, 0, 255), (255, 0, 0), (0, 255, 255)] 
while True:
    ret, frame = cap.read()
    counter = counter + 1
    ...
    if len(boxes_ids) > 0:
        ...
        # if len(frame_track) == 12:
        #     frame_track.pop(0)

        # print(len(frame_track))
        ...
        for box_id in boxes_ids:
            x, y, w, h, id = box_id
            track_object_id = id #prev_track_id
            ...
            for history in range(0, 11):
                ...
                # indexing_frame = (frame_track_.shape[0]-1) - history
            ...
            for index_run in range(3):
                ...
                for output_ in outputs:
                    if output_[0] + target_x_orig < 224 and output_[1] + target_y_orig < 224:
                        cv2.circle(roi, (int(output_[0] + target_x_orig), int(output_[1] + target_y_orig)), 1, color[index_run], 1)
            ...
            dim = (224*2, 224*2)
            roi_big = cv2.resize(roi, dim, interpolation =cv2.INTER_AREA)

            cv2.imshow("Detection Region", roi_big)
