...
HID_DIM = 128
N_LAYERS = 2
ENC_DROPOUT = 0 #0.2 #0.2
DEC_DROPOUT = 0 #0.2 #0.2
...
class Seq2Seq(nn.Module):
    ...
    def encoder_cnn(temp):
        # load pre-trained Conv2D model
        # '''
        resnet50_cnn = resnet50(pretrained=True, p=0)

        # change input channels number to match the rasterizer's output
        num_in_channels = 3 + (2*past_trajectory)
        resnet50_cnn.conv1 = nn.Conv2d(
            num_in_channels,
            resnet50_cnn.conv1.out_channels,
            kernel_size=resnet50_cnn.conv1.kernel_size,
            stride=resnet50_cnn.conv1.stride,
            padding=resnet50_cnn.conv1.padding,
            bias=False,
        )
        # change output size to (X, Y) * number of future states
        # num_targets = 2 * (sequence_length - past_trajectory)
        resnet50_cnn.fc = nn.Linear(in_features=2048, out_features=HID_DIM)

        return resnet50_cnn.to(config.DEVICE)

    # def forward(self, image, trg, teacher_forcing_ratio = 0.5):
    def forward(self, image, trg, teacher_forcing_ratio = 1):
        ...
        encoder_model = self.encoder_cnn()
        hidden = encoder_model(image)
...
def fit(model, dataloader, data, epoch):
    ...
    for i, data in tqdm(enumerate(dataloader), total=num_batches):
        ...
        image, keypoints, availability, history_traj,  = data['image'].to(config.DEVICE), torch.squeeze(data['keypoints'].to(config.DEVICE)), torch.squeeze(data['availability'].to(config.DEVICE)), torch.squeeze(data['history_traj'].to(config.DEVICE))
...
def validate(model, dataloader, data, epoch):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=num_batches):
            ...
            outputs = model(image, keypoints, 0)
            outputs = torch.transpose(outputs, 0, 1)
            outputs = outputs.reshape(outputs.size(0), -1)
